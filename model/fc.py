...
class GroupMLP(nn.Module):
    def __init__(self, in_features, mid_features, out_features, drop=0.5, groups=1):
        ...
        # [5120, 8192, kernel_size=(1,), stride=(1,)]
        self.conv1 = nn.Conv1d(in_features, mid_features, 1)
        self.drop = nn.Dropout(p=drop)
        self.relu = nn.LeakyReLU()
        # groups 是卷积层中的分组数 [8192, 1024, kernel_size=(1,), stride=(1,), groups=64]
        self.conv2 = nn.Conv1d(mid_features, out_features, 1, groups=groups)

    # 将输入张量a从2D转换为3D张量，以便可以应用卷积层
    # 最后，将输出张量重新转换为 2D 张量
    def forward(self, a):
        N, C = a.size()                             # [128, 5120]
        h = self.relu(self.conv1(a.view(N, C, 1)))  # [128, 8192, 1]
        return self.conv2(self.drop(h)).view(N, -1) # [128, 1024]
