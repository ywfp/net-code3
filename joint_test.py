...
class Runner:
    def __init__(self, args):
        ...
        self.fusion_model_rel, self.answer_net_rel = self._model_choice_rel(args)
        self.fusion_model_fact, self.answer_net_fact = self._model_choice(args)

        # get the mask from zsl
        self.negtive_mux = unseen_mask(args, self.val_loader) # unseen 类置0，seen 类负无穷
    ...
    def run(self):
        ...
        for method_choice in self.method_list:
            ...
            # L2归一化处理缩放到单位长度，消除向量长度对相似性度量的影响
    ...
    def eval(self):
        ...
        for visual_features, boxes, question_features, answers, idx, q_len in tq:
            with torch.no_grad():
                ...
                fusion_embedading_rel = self.fusion_model_rel(question_features)
                ...
                # 在每个批次迭代中，记录损失、真实答案和预测值，并更新训练指标
                ...
                if self.args.joint_test_way == 0:  # way 0
                    ...
                    # add mask实现对每个事实-关系对应的正确答案的屏蔽，降低topk以外ans的概率值
        ...
        self.val_metrics_orig.update_per_epoch() # 更新所有训练指标
    ...
    def get_min_and_max(self, predicted):
        ...
        # ok:[batch_size, 500] 每个样本最可能的500个answer及其得分
        ok, _ = predicted.topk(500, dim=1) # 从predicted的每一行中选取排序后前k个
        # 1 * batch_size矩阵 每个样本的预测结果
        min_tmp = ok[:, -1].reshape(1, -1) # 选取最小将其reshape成1行，列数与原始predicted相同
    ...
    # 解释模型在测试集上预测结果，输出该问题的id、name、question、gt answer、topk_ans、topk_rel、topk_fact等
    ...
    def _model_choice_rel(self, args):
        # models api
        fusion_model = getattr(fusion_net, 'LSTMEncoder')(args, self.question_word2vec).cuda()
        assert args.answer_embedding in ['MLP']
        answer_model = getattr(answer_net, args.answer_embedding)(args, self.train_loader.dataset).cuda()
        return fusion_model, answer_model
    ...
    def _load_model(self, model, function, type_name):
        ...
        target = type_name
        if function == "fusion" and type_name == "relation":
        ...
        save_path1 = os.path.join(save_path, self.args.fusion_model)
        
        save_path2 = os.path.join(save_path1, f'{target}_{model_name}_{self.args.FVQA.data_choice}.pkl')
        ...
        print(f"loading {save_path2} model done!")
    ...
    # 遮盖预测答案向量的掩码(根据rel和fact向量确定)，使得每个事实-关系对应的正确答案不被选择
                ...
                for j in range(self.args.top_fact):
                    ...
                    if name in fact_relation_to_ans.keys():
                        for k in fact_relation_to_ans[name]:
                            ...
                            # 将对应答案的位置标记为0以便后续计算不会将这些答案选为最终答案
                            mask_ans[item][k] = 0 
