...
class cfg():
    def __init__(self):
        ...
        self.FVQA.common_data_path = osp.join(self.FVQA.exp_data_path, "common_data")           # /data/KG-VQA/fvqa/exp_data/common_data
        self.FVQA.test_data_path = osp.join(self.FVQA.exp_data_path, "test_data")               # /data/KG-VQA/fvqa/exp_data/test_data
        self.FVQA.train_data_path = osp.join(self.FVQA.exp_data_path, "train_data")             # /data/KG-VQA/fvqa/exp_data/train_data
        self.FVQA.seen_train_data_path = osp.join(self.FVQA.exp_data_path, "train_seen_data")   # /data/KG-VQA/fvqa/exp_data/train_seen_data
        self.FVQA.unseen_test_data_path = osp.join(self.FVQA.exp_data_path, "test_unseen_data") # /data/KG-VQA/fvqa/exp_data/test_unseen_data
        self.FVQA.seen_test_data_path = osp.join(self.FVQA.exp_data_path, "test_seen_data")     # /data/KG-VQA/fvqa/exp_data/test_seen_data
        self.FVQA.model_save_path = osp.join(self.FVQA.data_path, "model_save")                 # /data/KG-VQA/fvqa/model_save
        self.FVQA.runs_path = osp.join(self.FVQA.data_path, "model_save")

        self.FVQA.qa_path = self.FVQA.exp_data_path
 
        self.FVQA.feature_path = osp.join(self.FVQA.common_data_path, 'fvqa-resnet-14x14.h5')                       # /data/KG-VQA/fvqa/exp_data/common_data/resnet-14x14.h5
        self.FVQA.answer_vocab_path = osp.join(
            self.FVQA.common_data_path, 'answer.vocab.fvqa.' + str(self.FVQA.max_ans) + '.json')                    # /data/KG-VQA/fvqa/exp_data/common_data/answer.vocab.fvqa.500.json
        self.FVQA.fact_vocab_path = osp.join(self.FVQA.common_data_path, 'answer.vocab.fvqa.fact.500.json')         # /data/KG-VQA/fvqa/exp_data/common_data/answer.vocab.fvqa.fact.500.json
        self.FVQA.relation_vocab_path = osp.join(self.FVQA.common_data_path, 'answer.vocab.fvqa.relation.500.json') # /data/KG-VQA/fvqa/exp_data/common_data/answer.vocab.fvqa.relation.500.json

        self.FVQA.fact_relation_to_ans_path = osp.join(self.FVQA.common_data_path, "fact_relation_dict.data") # /data/KG-VQA/fvqa/exp_data/common_data/fact_relation_dict.data
        self.FVQA.img_path = osp.join(self.FVQA.qa_path, 'images')                                            # /data/KG-VQA/fvqa/exp_data/images

        self.FVQA.kg_path = osp.join(self.FVQA.common_data_path, "KG_embedding")      # /data/KG-VQA/fvqa/exp_data/common_data/KG_embedding
        self.FVQA.gae_path = osp.join(self.FVQA.common_data_path, "GAE_embedding")    # /data/KG-VQA/fvqa/exp_data/common_data/GAE_embedding
        self.FVQA.bert_path = osp.join(self.FVQA.common_data_path, "BERT_embedding")  # /data/KG-VQA/fvqa/exp_data/common_data/BERT_embedding
        ...
        # mydata
        # self.train_path = osp.join(self.FVQA.img_path, "images")
        # self.preprocessed_path = osp.join(self.FVQA.common_data_path, "resnet-14x14.h5")


        self.cache_path = osp.join(self.data_root, '.cache') # /data/KG-VQA/.cache
        ...
        # a joint question vocab across all dataset           /data/KG-VQA/fvqa/exp_data/common_data/question.vocab.json
        self.question_vocab_path = osp.join(self.FVQA.common_data_path, 'question.vocab.json')  # 修改这里之后所有的预存文件（pt）都要删除

        # preprocess config
        self.image_size = 448
        self.output_size = self.image_size // 32  # 448//32=14
        ...
        self.TRAIN.answer_batch_size = self.FVQA.max_ans  # batch size for answer network              500
        self.TRAIN.max_negative_answer = self.FVQA.max_ans  # max negative answers to sample           500

        # Test params
        self.TEST = edict()
        # self.TEST.batch_size = 128
        self.TEST.batch_size = 128
        self.TEST.max_answer_index = self.FVQA.max_ans  # max answer index for computing acc   853     500
    ...
    def get_args(self):
        ...
        parser.add_argument('--max_ans', default=22, type=int)  # 3000 300##
        ...
        # choice model
        parser.add_argument('--fusion_model', default='SAN', choices=['MLP', 'SAN', 'UD', 'MUTAN', 'BAN', 'ViLBERT', 'LSTMEncoder'])
        ...
        # parser.add_argument("--random_seed", default=3456, type=int)
    ...
    def update_train_configs(self, args):
        ...
        if args.ZSL == 1:
            print("ZSL setting...")
            self.FVQA.test_data_path = self.FVQA.unseen_test_data_path  # /data/KG-VQA/fvqa/exp_data/test_unseen_data
            self.FVQA.train_data_path = self.FVQA.seen_train_data_path  # /data/KG-VQA/fvqa/exp_data/train_seen_data

        if args.fusion_model == 'UD' or args.fusion_model == 'BAN':
            self.FVQA.feature_path = osp.join(self.FVQA.common_data_path, 'fvqa_36.hdf5')       # /data/KG-VQA/fvqa/exp_data/common_data/fvqa_36.hdf5
            self.FVQA.img_id2idx = osp.join(self.FVQA.common_data_path, 'fvqa36_imgid2idx.pkl') # /data/KG-VQA/fvqa/exp_data/common_data/fvqa36_imgid2idx.pkl
        ...
        if self.fact_map:
            self.FVQA.max_ans = 21
        if self.relation_map:
            self.FVQA.max_ans = 10
        ...
        if "KG" in self.method_choice:
            self.FVQA.relation2id_path = osp.join(self.FVQA.kg_path, "relations_" + self.FVQA.entity_num + ".tsv") # /data/KG-VQA/fvqa/exp_data/common_data/KG_embedding/relations_all.tsv
            self.FVQA.entity2id_path = osp.join(self.FVQA.kg_path, "entities_" + self.FVQA.entity_num + ".tsv")    # /data/KG-VQA/fvqa/exp_data/common_data/KG_embedding/entities_all.tsv
            if self.KGE_init != "w2v":
                self.FVQA.entity_path = osp.join(self.FVQA.kg_path, "fvqa_" +
                                                 self.FVQA.entity_num + "_" + self.KGE + "_entity.npy")            # /data/KG-VQA/fvqa/exp_data/common_data/KG_embedding/fvqa_all_TransE_entity.npy
                self.FVQA.relation_path = osp.join(self.FVQA.kg_path, "fvqa_" +
                                                   self.FVQA.entity_num + "_" + self.KGE + "_relation.npy")        # /data/KG-VQA/fvqa/exp_data/common_data/KG_embedding/fvqa_all_TransE_relation.npy
            else:
                self.FVQA.entity_path = osp.join(self.FVQA.kg_path, "fvqa_" +
                                                 self.FVQA.entity_num + "_w2v_" + self.KGE + "_entity.npy")        # /data/KG-VQA/fvqa/exp_data/common_data/KG_embedding/fvqa_all_w2v_TransE_entity.npy
                self.FVQA.relation_path = osp.join(self.FVQA.kg_path, "fvqa_" +
                                                   self.FVQA.entity_num + "_w2v_" + self.KGE + "_relation.npy")    # /data/KG-VQA/fvqa/exp_data/common_data/KG_embedding/fvqa_all_w2v_TransE_relation.npy
